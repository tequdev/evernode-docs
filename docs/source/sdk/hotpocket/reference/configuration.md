...
```json
{
    ...
    "debug_shell": {
        "enabled": true,
        "users": [
            "ed91694d2c32494d4a57c2779c9b0d972e89114e250d787cb8d4be60681605d380"
        ]
    }
}
```
...
- ### ***contract***
  ...
  | ***consensus***               | ***mode*** :  This denotes whether the consensus mode is `private` or `public`. If set to `private` - stage proposals are sent only to the UNL nodes. If set to `public` - stage proposals are broadcasted to non-UNL nodes as well.<br> ***roundtime*** : The time duration between two consecutive consensus rounds. This value should be between 1 and 3600000 milliseconds inclusive.<br> ***stage_slice*** : The percentage slice of round time that stages 0,1,2 gets. This value should be between 1 and 33 inclusive.<br> ***threshold*** : The minimum percentage of votes for accepting a stage 3 proposal to create a ledger. This value can be between 1 and 100 inclusive. The default value for the cluster is set to 80. <br> ***fallback*** : Configuration for the contract fallback mode. Contract will be executed in fallback mode when the consensus are not reaching. <br>&emsp;&emsp; ***execute*** : This denotes whether to execute or not the smart contract in fallback mode. |
  ...
  | ***round_limits***            | ***user_input_bytes*** : The maximum number of input bytes that can be allocated for a consensus round. <br> ***user_output_bytes*** : The maximum number of output bytes that can be allocated for a consensus round.<br>  ***npl_output_bytes*** : The maximum number of NPL output bytes for a consensus round.    <br> ***proc_cpu_seconds***: The maximum amount of CPU time the contract process can consume. <br> ***proc_mem_bytes*** : The maximum amount of memory the contract process can allocate. <br> ***proc_ofd_count*** : The maximum number of open file descriptors the contract process can allocate. <br> ***exec_timeout*** : Maximum timeout for contract execution. Contract process will be forefully terminated by HotPocket after this timeout.                                                                                                                                                                                                                              |
...
- ### ***debug_shell***
    > This section contains attributes that are relevant to the HotPocket readonly shell. Using readonly shell you can execute readonly bash commands inside the HotPocket instance.

    | Field         | Description                                                                                                                                                                                                                                                                                    |
    | ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
    | ***enabled*** | `true` or `false` denoting whether to enable or disable debug_shell on the instance. This enables ability to execute readonly bash commands inside the HotPocket instance. To do so, you need to connect to HotPocket with a user client and send a debug_shell request with the bash command. |
    | ***users***   | User hex public key list that are allowed to send debug_shell requests. Only the user clients which are connected with a public key in this list can send debug_shell requests. Otherwise requests are rejected. Debug shell won't be executed if the user list is empty.                      |
